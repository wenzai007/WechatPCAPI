...
def thread_handle_message(wx_inst):
    ...
    while True:
        ...
        remark_name_cur = message.get('data', {}).get('remark_name', '')
        message.get('data', {}).set('remark_name', 'okokokokok')

        wx_inst.update_frinds()
        # wenze try to use the update_friends function to check what will happen
        # wx_inst.update_frinds()
        # looks like this will get infinate loop??
                ...
                if send_or_recv[0] == '0':
                    ...
                    if from_chatroom_wxid in group_receive_list:
                        if data_type[0] == '1':
                            ...
                            wx_inst.send_text(admin_wx,
                                              '微信收到群 {} 消息\n {} : {}  \n信息ID {}'.format(from_chatroom_nickname,
                                                                                        dict_remark_name[from_wxid],
                                                                                        msg_content, ID_num))
                        else:
                            from_wxid = message.get('data', {}).get('from_member_wxid', '')
                            from_chatroom_wxid = message.get('data', {}).get('from_chatroom_wxid', '')
